...
if __name__ == '__main__':
    ...
    data_order = data[['num', 'X', 'Y', 'Z', 'type', 'type_q3']]
    ...
    data_order = data_order[data_order['d'].values <= d_max]
    ...
    num_index = {}
    # data_order.to_csv('data_order.csv')
    ...
    q3type_cal = data_order.type_q3.values[1:-1]
    loc_cal = [x_cal, y_cal, z_cal]
    primary_population_size = 20
    chromosome_length = len(loc_cal[0])
    population_size = 20

    population = species_origin(num_index, distance_dict, primary_population_size, chromosome_length)
    pc = 0.25
    pm = 0.02
    best_length = []
    best_num_node = []
    ...
    for i in range(50):
        ...
        fitness,df = function(loc_b,loc_a,loc_cal,type_cal,population,chromosome_length,q3type_cal)
    ...
    error_level_list, error_vertical_list = best_error(loc_a, loc_b, loc_cal, type_cal, bestindividual, q3type_cal)

    result = pd.DataFrame({'校正点编号': best_num,
                           '校正前垂直误差': error_vertical_list,
                           '校正前水平误差': error_level_list,
                           '校正点类型': best_type_list})
    result.to_csv('result3_1终稿.csv', encoding='gbk')
